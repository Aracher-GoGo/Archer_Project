...
namespace CMF
{
    ...
    public class CharacterJoystickInput : CharacterInput, IPointerDownHandler, IDragHandler, IPointerUpHandler 
    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            //화면 공간으로 변환
            Vector2 _position = RectTransformUtility.WorldToScreenPoint(cam, background.position);
            //반지름 설정
            Vector2 _radius = background.sizeDelta / 2;
            input = (eventData.position - _position) / (_radius * canvas.scaleFactor);
        
            FormatInput();
            HandleInput(input.magnitude, input.normalized, _radius, cam);
            handle.anchoredPosition = input * _radius * handleRange;
        }
        ...
        protected Vector2 ScreenPointToAnchoredPosition(Vector2 screenPosition)
        {
            ...
            if (RectTransformUtility.ScreenPointToLocalPointInRectangle(baseRect, screenPosition, cam, out _localPoint))
            {
                Vector2 _pivotOffset = baseRect.pivot * baseRect.sizeDelta;
                return _localPoint - (background.anchorMax * baseRect.sizeDelta) + _pivotOffset;
            }
            return Vector2.zero;
        }
