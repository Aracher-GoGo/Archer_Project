...
namespace ArcheryDemo
{
    public class TargetSystem : MonoBehaviour
    {
        ...
        public LayerMask defaultLayerMask; //허용되지 않는 유형 (ex 벽 장애물)
        public List<LayerMask> allowedCollisionTypes; //허용되는 유형
        public List<LayerMask> blockingCollisionTypes; //허용되지 않는 유형 (ex 벽 장애물)
    
        public float targetingMaxDistance = 300.0f;
        public float traceHeightOffset = 1.0f;
        public float traceDepthOffset = 1.0f;
        ...
        void Start()
        {
            //레이어마스크의 제대로 된 개념 [언리얼과 비교해서 너무 혼동옴]
            foreach (LayerMask _allowedCollisionType in allowedCollisionTypes)
                blockingCollisionTypes.Remove(_allowedCollisionType);
        ...
        {
            if (blockingCollisionTypes.Count > 0)
            {
                RaycastHit _hitData;
            
                Ray _ray = new Ray(GetRayOrigin(), _target.transform.position - transform.position);
                bool _isHit = Physics.Raycast( _ray,out _hitData,targetingMaxDistance,defaultLayerMask);

                if (_isHit)
                {
                    //! (허용되지 않는 유형들에 포함되어 있지 않고 (그리고) 제외해야할 오브젝트에 포함되어 있지 않다면 = true)
                    return !(!blockingCollisionTypes.Contains(_hitData.transform.gameObject.layer) && !_objectToIgnores.Contains(_hitData.transform.gameObject));
                }
        ...
        void OnDrawGizmos ( )
        {
            if (selectedTarget!= null)
            {
                ...
                // bool _isHit = Physics.Raycast( _ray,out _hit,targetingMaxDistance,defaultLayerMask);
                bool _isHit = Physics.Raycast ( GetRayOrigin(), selectedTarget.transform.position - transform.position, out _hit, targetingMaxDistance, defaultLayerMask );
