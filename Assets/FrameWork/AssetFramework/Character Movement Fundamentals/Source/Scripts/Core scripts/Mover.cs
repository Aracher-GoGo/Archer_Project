...
namespace CMF
{
    //이 스크립트는 모든 물리, 충돌 감지 및 지면 감지를 처리합니다.
    //(컨트롤러 스크립트와 같은) 외부 스크립트의 'FixedUpdate' 프레임마다 이동 속도('SetVelocity'를 통해)가 작동할 것으로 예상합니다.
    //또한 중요한 정보에 대한 여러 getter 메서드를 제공합니다(무버가 접지되었는지 여부, 현재 표면 노멀 [...]).
    public class Mover : MonoBehaviour 
    {
        ...
        void Awake()
        {
            ...
            RecalculateColliderDimensions(); //충돌체 치수 재계산
			
            RecalibrateSensor(); //센서 재보정
        }
